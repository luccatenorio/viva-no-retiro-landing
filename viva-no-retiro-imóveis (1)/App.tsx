    ...
    <div className="min-h-screen bg-white font-sans text-gray-900">
      ...
      >
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          {/* FIX: troca correta de versão da logo */}
          <div className="transition-all">
            <Logo className={isScrolled ? "h-12" : "h-12 brightness-0 invert"} />
          </div>
          
          {/* Desktop Nav */}
          <nav className={`hidden md:flex items-center gap-8 font-medium ${isScrolled ? 'text-gray-800' : 'text-white'}`}>
            <a href="#" className="hover:text-brand-green transition-colors">Sobre</a>
            <a href="#" className="hover:text-brand-green transition-colors">O Retiro</a>
            <a href="#" className="hover:text-brand-green transition-colors">Imóveis</a>
            <button className="bg-brand-green hover:bg-lime-500 text-brand-dark px-6 py-2 rounded-full font-bold transition-transform hover:scale-105">
              Fale Conosco
            </button>
          </nav>
        ...
        {isMobileMenuOpen && (
          <div className="absolute top-full left-0 right-0 bg-white border-t p-6 md:hidden shadow-xl flex flex-col gap-4 items-center text-lg">
            <a href="#" className="text-gray-800 py-2" onClick={() => setIsMobileMenuOpen(false)}>Sobre</a>
            <a href="#" className="text-gray-800 py-2" onClick={() => setIsMobileMenuOpen(false)}>O Retiro</a>
            <a href="#" className="text-gray-800 py-2" onClick={() => setIsMobileMenuOpen(false)}>Imóveis</a>
            <button className="w-full bg-brand-green text-brand-dark font-bold py-3 rounded-lg">
              Agendar Visita
            </button>
          </div>
        )}
      ...
      <main>
        ...
        <section id="contact" className="py-24 bg-gradient-to-br from-gray-50 to-gray-200 flex flex-col items-center text-center px-6">
          ...
          <button className="bg-brand-dark text-white text-xl font-bold px-10 py-5 rounded-full hover:bg-brand-petrol transition-all shadow-lg hover:shadow-2xl flex items-center gap-3">
